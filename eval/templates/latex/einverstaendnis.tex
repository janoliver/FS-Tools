...
\BLOCK{else} % tutor oder dozent

\BLOCK{for fragenset in vl.fragebogen.fragensets.all() -}
  \BLOCK{- for frage in fragenset.fragenn(fragen) -}
    \BLOCK{- if frage.fragentyp.texttype and frage.id == 34}
      \BLOCK{if vl.tutoren.all()|count > 0}
          \BLOCK{if frage.antworten(vl, dozent[1])|count > 0 -}
            ...
            {\bf \VAR{frage.text}}\\
            Tutor: \VAR{dozent[1].name}
      
            \begin{itemize}
            \BLOCK{for k in frage.antworten(vl, dozent[1])}
              \item \VAR{k.text|latexify}
            \BLOCK{endfor}
            \end{itemize}
          \BLOCK{endif}
