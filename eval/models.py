...
class Personal(cmodels.Timestamped_Model):
    ...
    personaltyp = models.ForeignKey('Personaltyp', blank=True,
                                    null=True,
...
class Vorlesung(cmodels.Timestamped_Model):
    ...
    def semester_dist(self):
        return [{'key': x['semester'], 'val': x['id__count']} for x in
                Antwortbogen.objects.filter(vorlesung=self,
                semester__isnull=False).values('semester'
                ).annotate(Count('id')).distinct()]

    def studiengang_dist(self):
        return [{'key': x['studiengang__name'], 'val': x['id__count']}
                for x in Antwortbogen.objects.filter(vorlesung=self,
                studiengang__isnull=False).values('studiengang__name'
                ).annotate(Count('id')).distinct()]

    def antworten(self):
        if not self.antworten_cache:
            self.antworten_cache = \
                Antwort.objects.select_related('frage', 'option'
...
class Antwortbogen(cmodels.Timestamped_Model):
    ...
    vorlesung = models.ForeignKey('Vorlesung',
                                  related_name='antwortboegen')
    studiengang = models.ForeignKey('Studiengang', blank=True,
                                    null=True,
                                    on_delete=models.SET_NULL)
...
class Frage(cmodels.Timestamped_Model):
    ...
    fragentyp = models.ForeignKey('Fragentyp', null=True,
                                  on_delete=models.SET_NULL)
    ...
    def antworten(self, vl, tutor=None):
        if not tutor:
            return Antwort.objects.filter(frage=self,
                    antwortbogen__vorlesung=vl)
        else:
            return Antwort.objects.filter(frage=self,
                    antwortbogen__vorlesung=vl,
                    antwortbogen__tutor=tutor)
    ...
    def total(self, rel):
        return len([x for x in rel.antworten() if x.frage_id
                   == self.id])
...
class Fragebogen(cmodels.Timestamped_Model):
    ...
    name = models.CharField(max_length=255)
    fragensets = models.ManyToManyField('Fragenset',
            through='FragensetFragebogen', related_name='frageboegen')
...
class Fragenset(cmodels.Timestamped_Model):
    ...
    fragen = models.ManyToManyField('Frage', related_name='fragensets',
                                    through='FrageFragenset')
...
class Vlu(cmodels.Timestamped_Model):
    ...
    def antworten(self):
        if not self.antworten_cache:
            self.antworten_cache = \
                Antwort.objects.select_related('frage', 'option'
                    ).filter(antwortbogen__vorlesung__vlu=self)
        return self.antworten_cache
...
class Option(cmodels.Timestamped_Model):
    ...
    def value(self, vl, frage):
        ...
        antworten_opt = len([x for x in vl.antworten() if x.option_id
                            == self.id and x.frage_id == frage.id])
...
class Antwort(cmodels.Timestamped_Model):
    ...
    antwortbogen = models.ForeignKey('Antwortbogen',
